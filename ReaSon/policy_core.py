...
class Trainer:
    ...
    def train_step(self,
        ...
        loss = self._compute_loss(sampled_groups, probs, group_rewards)
    ...
    def _compute_loss(self, sampled_groups, probs, group_rewards) -> torch.Tensor:
    ...
    def save_model(self, path: str):
        ...
        print(f"[Trainer] Policy model saved to {path}")

    def load_model(self, path: str):
        state_dict = torch.load(path, map_location='cpu')
        self.policy_net.load_state_dict(state_dict)
        print(f"[Trainer] Policy model loaded from {path}")
